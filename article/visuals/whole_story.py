...
fig.add_trace(go.Surface(
    x=lam_f, y=rs, z=Z, showscale=False,
    colorscale=[[0.0, "#123a52"], [0.02, "#1b6f9e"], [0.35, "#7a1f4f"],
                [1.0, "#ff2a6d"]],
    lighting=dict(ambient=0.66, diffuse=0.66, specular=0.15, roughness=0.75),
    contours=dict(z=dict(show=True, start=0, end=100, size=12,
                         color="#0b0714", width=1))))

# the shore: ratio exactly 1
fig.add_trace(go.Scatter3d(
    x=lam_f, y=[1.0] * len(lam_f), z=[1.2] * len(lam_f), mode="lines",
    line=dict(color="#f9f871", width=11), showlegend=False))

# the width-2 strip, in ratio units it is 2/shore -- thin, and thinner with lam
fig.add_trace(go.Scatter3d(
    x=lam_f, y=[1.0 - 2.0 / float(shore[x]) for x in LAMS],
    z=[0.8] * len(lam_f), mode="lines",
    line=dict(color="#4df0ff", width=8), showlegend=False))

# the string: lam = 1, D = 23, exactly on the shore
fig.add_trace(go.Scatter3d(
    x=[1.0], y=[1.0], z=[3.0], mode="markers",
    marker=dict(size=16, color="#ffffff", symbol="diamond"),
    showlegend=False))

# the knives
kx, ky, kz = [], [], []
for lam in (Fraction(3), Fraction(7), Fraction(10), Fraction(14),
            Fraction(18), Fraction(22), Fraction(26), Fraction(30)):
    sh = T_hat(lam)
    ...
    for j in (4, 6, 8, 10):
        ...
        if best is not None:
            ...
            if r > 1.28:            # outside the drawn range
                continue
            kx.append(float(lam)); ky.append(r)
            kz.append((r - 1.0) * 320 + 2.0)
fig.add_trace(go.Scatter3d(
    x=kx, y=ky, z=kz, mode="markers",
    marker=dict(size=6, color="#ff2a6d", line=dict(color="#ffd0dd", width=1)),
    showlegend=False))

ann = [
    dict(x=22, y=0.45, z=6, text="ALLOWED<br>gravity theories live here",
         font=dict(color="#8ff0ff", size=16), showarrow=False),
    dict(x=24, y=1.24, z=76,
         text="FORBIDDEN", font=dict(color="#ff8fb0", size=17),
         showarrow=False),
    dict(x=8, y=1.02, z=30,
    ...
    dict(x=1, y=1.0, z=54,
         ...
         font=dict(color="#ffffff", size=14), showarrow=False),
    dict(x=29, y=1.10, z=42,
         text="the knives cut only here, above the edge",
         font=dict(color="#ff8fb0", size=14), showarrow=False),
    dict(x=14, y=0.80, z=-9,
         text="cyan band = all that is left to prove",
         font=dict(color="#4df0ff", size=14), showarrow=False)]
...
fig.update_layout(
    ...
    title=dict(
        ...
        font=dict(color="#e8e6f0", size=21), x=0.025, y=0.955),
    scene=dict(
        xaxis=dict(title="lam — which family of theories",
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        yaxis=dict(title="dimension, measured against the shore",
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        zaxis=dict(title="how far into forbidden land", showticklabels=False,
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        aspectratio=dict(x=1.6, y=1.2, z=0.85),
        annotations=ann,
        camera=dict(eye=dict(x=1.5, y=-1.45, z=0.66),
                    center=dict(x=0, y=0, z=-0.08))),
    margin=dict(l=0, r=0, t=120, b=0), width=1460, height=700)
