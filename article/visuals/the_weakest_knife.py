...
def gather():
    ...
    for lam, D in ((F(1), F(6)), (F(7), F(6))):
        ...
        out["weakest"][key] = []
        for n in LEVELS:
            vals = [float(F(-1) ** m * jacobi_moment(n - m, n, lam, D)) for m in range(n - 1)]
...
fig = make_subplots(
    ...
    subplot_titles=("every knife of a level, relative size", "the weakest knife against the level"),
)

pal = ["#7df3ff", "#4df0ff", "#58d3be", "#c9e86b"]
for (n, rel), col in zip(
    sorted(d["profiles"].items(), key=lambda kv: int(kv[0])), pal, strict=False
):
...
fig.add_annotation(
    x=1.0,
    y=-3,
    text="leading trajectory j=2 (highest spin)",
    showarrow=True,
    arrowhead=2,
    ax=-70,
    ay=-30,
...
for (key, pts), col, name in zip(
    sorted(d["weakest"].items()), ("#4df0ff", "#ff8fb0"), ("lam = 1", "lam = 7"), strict=False
):
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
    fig.add_trace(
...
fig.update_layout(
    ...
    xaxis=dict(
        title="m / (n-2)   — 1 is the LEADING trajectory j=2, 0 is spin zero",
        color="#a8a08e",
        gridcolor="#322e26",
    ),
    yaxis=dict(
        title="log10 of size relative to the largest knife", color="#a8a08e", gridcolor="#322e26"
    ),
    xaxis2=dict(title="n — the level", color="#a8a08e", gridcolor="#322e26"),
    yaxis2=dict(title="log10 of the weakest coefficient", color="#a8a08e", gridcolor="#322e26"),
    legend=dict(
    ...
    margin=dict(l=80, r=30, t=140, b=100),
    width=1540,
    height=680,
)
