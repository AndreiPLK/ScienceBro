...
for n, l, h in bars:
    fig.add_trace(
        go.Mesh3d(
            ...
            opacity=1.0,
            showlegend=False,
            lighting=dict(ambient=0.45, diffuse=0.6, specular=0.35, roughness=0.4),
...
fig.update_layout(
    ...
    title=dict(
        text="THE CITY OF STRING PARTICLES<br>"
    ...
    scene=dict(
        xaxis=dict(title="level n (mass ladder)", color="#5c5870", gridcolor="#150e24"),
        yaxis=dict(title="spin l", color="#5c5870", gridcolor="#150e24"),
        zaxis=dict(title="", showticklabels=False, color="#5c5870", gridcolor="#150e24"),
        camera=dict(eye=dict(x=1.35, y=-1.5, z=0.55), center=dict(x=0, y=0, z=-0.1)),
        aspectmode="manual",
...
Image.open(out / "_c_raw.png").resize((1600, 1000), Image.LANCZOS).save(out / "string-city.png")
