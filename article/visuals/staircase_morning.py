...
def knife_pct(j):
    checks = [(f"knife_proof2_j{j}.json", "ok_so_far", 25),
...
fig.add_trace(go.Surface(
    x=xs, y=ys, z=zs,
    colorscale=[[0, "#33091d"], [0.3, "#ff2a6d"], [0.55, "#7a3fd0"],
                [0.8, "#0aa3c2"], [1, "#3fe7f5"]],
    showscale=False, lighting=dict(ambient=0.5, diffuse=0.75, specular=0.2)))

done = [j for j in KN if H[j] == 100]
fig.add_trace(go.Scatter3d(
    x=[22] * len(done), y=done, z=[H[j] / 6.5 + 0.5 for j in done],
    mode="markers", marker=dict(size=8, color="#f9f871", symbol="diamond"),
    showlegend=False))
fig.add_trace(go.Scatter3d(
    x=[22], y=[max(done) + 0.2], z=[H[max(done)] / 6.5 + 2.2],
    mode="text", text=[f"PROVEN THEOREMS: knives {min(done)}-{max(done)}"],
    textfont=dict(color="#f9f871", size=17), showlegend=False))

ann = [dict(x=8, y=13.2, z=17.5, text="THE STAIRCASE — SEVEN PROVEN KNIVES",
            font=dict(color="#9ff5ff", size=24), showarrow=False),
       dict(x=30, y=10.5, z=6.5,
            text="knives 9-11: 3 of 4 regions proven (80%);<br>"
                 "far-below hits the method boundary found tonight",
            font=dict(color="#ff5f95", size=13), showarrow=False)]

fig.update_layout(
    template="plotly_dark", paper_bgcolor="#0b0714",
    title=dict(text="THE STAIRCASE TO THE GRAND THEOREM — morning 17.08<br>"
    ...
    scene=dict(
        ...
        yaxis=dict(title="knife j", color="#8f8ba0", gridcolor="#241a38",
                   dtick=1),
        zaxis=dict(title="proven", color="#8f8ba0", gridcolor="#241a38",
                   range=[0, 19], showticklabels=False),
        annotations=ann,
        camera=dict(eye=dict(x=1.75, y=-1.5, z=0.7),
