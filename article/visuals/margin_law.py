    ...
    for j in JS:
        for lam_i in LAMS:
            ...
            if best is not None:
                ...
                print(f"  j={j} lam={lam_i}: gap={float(best-shore):.4f}",
                      flush=True)
...
fig.add_trace(go.Surface(
    x=LAMS, y=JS, z=Z, showscale=False,
    colorscale=[[0.0, "#241033"], [0.35, "#155f8a"], [0.7, "#3fe7f5"],
                [1.0, "#f9f871"]],
    lighting=dict(ambient=0.6, diffuse=0.7, specular=0.2, roughness=0.7),
    contours=dict(z=dict(show=True, start=0, end=26, size=2.4,
                         color="#0b0714", width=1))))

# the predicted terraces: 2.398 (j - 2), flat in lam
for j in JS:
    fig.add_trace(go.Scatter3d(
        x=LAMS, y=[j] * len(LAMS), z=[2.398 * (j - 2)] * len(LAMS),
        mode="lines", line=dict(color="#ff2a6d", width=5),
        showlegend=False))

ann = [dict(x=LAMS[-1], y=12, z=2.398 * 10 + 3.4,
            text="red lines = 2.398 (j - 2)<br>the measured law",
            font=dict(color="#ff8fb0", size=14), showarrow=False),
       dict(x=LAMS[2], y=4, z=7.5,
...
fig.update_layout(
    ...
    title=dict(
        ...
        font=dict(color="#e8e6f0", size=20), x=0.025, y=0.955),
    scene=dict(
        xaxis=dict(title="lam — deformation of the amplitude",
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        yaxis=dict(title="j — which knife", dtick=2,
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        zaxis=dict(title="clearance above the shore (dimensions)",
                   backgroundcolor="#0b0714", gridcolor="#2a2140",
                   color="#b9b4cc"),
        aspectratio=dict(x=1.6, y=1.25, z=0.6),
        annotations=ann,
        camera=dict(eye=dict(x=1.55, y=-1.35, z=0.62),
                    center=dict(x=0, y=0, z=-0.12))),
    margin=dict(l=0, r=0, t=104, b=0), width=1480, height=600)
