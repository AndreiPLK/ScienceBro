...
fig.add_trace(
    go.Scatter3d(
        x=[p[0] for p in floor_touch],
        y=[p[1] for p in floor_touch],
        z=[0.02] * len(floor_touch),
        mode="markers",
        marker=dict(size=4, color="#f9f871"),
        showlegend=False,
    )
)
...
ann = [
    dict(
        ...
        y=17.5,
        z=3.4,
        text="THE PERFECT SQUARE",
        font=dict(color="#9ff5ff", size=25),
        showarrow=False,
    ),
    dict(
        x=0.030,
    ...
    dict(
        ...
        z=0.06,
        text="ZERO FLOOR",
        font=dict(color="#ff5f95", size=15),
        showarrow=False,
    ),
]
...
fig.update_layout(
    ...
    title=dict(
        text="THE PERFECT SQUARE — why the sign problem is NOT in the "
        ...
        font=dict(color="#e8e6f0", size=19),
        x=0.03,
    ),
    scene=dict(
        xaxis=dict(title="x", color="#8f8ba0", gridcolor="#241a38"),
        yaxis=dict(title="level n", color="#8f8ba0", gridcolor="#241a38"),
        zaxis=dict(title="", color="#8f8ba0", gridcolor="#241a38", showticklabels=False),
        camera=dict(eye=dict(x=1.7, y=-1.5, z=0.7), center=dict(x=0, y=0, z=-0.1)),
    ),
    width=1600,
    height=1000,
...
Image.open(out / "_sq_raw.png").resize((1600, 1000), Image.LANCZOS).save(out / "perfect-square.png")
