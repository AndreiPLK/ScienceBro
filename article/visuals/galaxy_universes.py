...
STARS = [[0.0, "#7a3fd0"], [0.45, "#0aa3c2"], [0.8, "#3fe7f5"], [1.0, "#eafffe"]]
fig = go.Figure()
fig.add_trace(
    go.Scatter3d(
        x=dx,
        ...
        z=dz,
        mode="markers",
        marker=dict(size=1.6, color="#5c1330", opacity=0.5),
        showlegend=False,
    )
)
fig.add_trace(
    go.Scatter3d(
        ...
        z=zs,
        mode="markers",
        marker=dict(size=sizes, color=cols, colorscale=STARS, opacity=0.9, line=dict(width=0)),
        showlegend=False,
    )
)
...
fig.add_trace(
    go.Scatter3d(
        ...
        z=[0.35],
        mode="markers+text",
        marker=dict(size=9, color="#f9f871", symbol="diamond"),
...
fig.update_layout(
    ...
    title=dict(
        text="THE GALAXY OF SURVIVING UNIVERSES<br>"
    ...
    scene=dict(
        xaxis=dict(
            title="which theory (lambda)",
            color="#5c5870",
            gridcolor="#150e24",
            showbackground=False,
        ),
        yaxis=dict(
            title="dimensions of the world (D)",
            color="#5c5870",
            gridcolor="#150e24",
            showbackground=False,
        ),
        ...
        camera=dict(eye=dict(x=1.05, y=-1.25, z=0.35), center=dict(x=0, y=0, z=-0.05)),
    ),
    width=1600,
    height=1000,
