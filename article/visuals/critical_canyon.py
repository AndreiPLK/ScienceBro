...
fig.add_trace(
    go.Surface(
        ...
        z=Z,
        showscale=False,
        colorscale=[[0.0, "#ff2a6d"], [0.35, "#7a3fd0"], [0.7, "#0aa3c2"], [1.0, "#3fe7f5"]],
...
for i, j in enumerate(js):
    ...
    xs.append(np.log10(lams[k]))
    ys.append(j)
    zs.append(Z[i, k] + 0.06)
fig.add_trace(
    go.Scatter3d(
        x=xs,
        y=ys,
...
ann = [
    dict(
        ...
        font=dict(color="#3fe7f5", size=14),
        showarrow=False,
    ),
    dict(
        x=np.log10(9),
        y=js[len(js) // 2],
        z=-4.35,
        text="THE CRITICAL CANYON — margin 0.0002<br>this is where the grand theorem is decided",
        font=dict(color="#ffe94a", size=14),
        showarrow=False,
    ),
    dict(
        x=np.log10(1.1),
...
fig.update_layout(
    ...
    title=dict(
        text="THE CRITICAL CANYON — where the last theorem lives<br>"
    ...
    scene=dict(
        xaxis=dict(title="log10(lambda)", color="#8f8ba0", gridcolor="#241a38"),
        yaxis=dict(title="knife j", color="#8f8ba0", gridcolor="#241a38", dtick=2),
        zaxis=dict(title="log10(margin)", color="#8f8ba0", gridcolor="#241a38"),
        annotations=ann,
        camera=dict(eye=dict(x=1.6, y=-1.5, z=0.75), center=dict(x=0, y=0, z=-0.1)),
    ),
    width=1600,
    height=1000,
