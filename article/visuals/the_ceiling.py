...
CASES = [(31, F(1), F(6), "#4df0ff"), (31, F(1), F(11), "#c9e86b"),
         (21, F(7), F(6), "#ff8fb0")]
...
def gather():
    ...
    for n, lam, D, _ in CASES:
        ...
        for t, c in enumerate(roots):
            ...
            H = mul_square(H, c)
            print("  n=%d lam=%s D=%s t=%d ceiling %.5f ladder %.5f"
                  % (n, lam, D, t, float(cm), float(c)), flush=True)
...
for n, lam, D, col in CASES:
    ...
    fig.add_trace(go.Scatter(
        x=ts, y=d["ceiling"], mode="lines", name=f"ceiling: n={n}, lam={lam}, D={D}",
        line=dict(color=col, width=3)))
    fig.add_trace(go.Scatter(
        x=ts, y=d["ladder"], mode="markers", showlegend=True,
        name=f"the ladder the problem asks for (n={n}, lam={lam})",
        marker=dict(color=col, size=11, symbol="square",
                    line=dict(color="#14130f", width=1))))
...
fig.update_layout(
    ...
    xaxis=dict(title="t — how many factors are already in the product",
               color="#a8a08e", gridcolor="#322e26", dtick=1),
    yaxis=dict(title="root position c  (the interval is 0 to 1)",
               color="#a8a08e", gridcolor="#322e26", range=[0, 1.02]),
    legend=dict(font=dict(color="#a8a08e", size=10), x=0.02, y=0.98,
                bgcolor="rgba(20,19,15,0.75)"),
    margin=dict(l=80, r=30, t=140, b=70), width=1480, height=720)
