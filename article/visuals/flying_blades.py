...
fig.add_trace(go.Isosurface(
    ...
    isomin=-0.001, isomax=0.001, surface_count=1,
    colorscale=[[0, "#ff2a6d"], [1, "#ff2a6d"]], opacity=0.72,
    caps=dict(x_show=False, y_show=False, z_show=False), showscale=False))
fig.add_trace(go.Isosurface(
    x=X.flatten(), y=Y.flatten(), z=Z.flatten(), value=shore.flatten(),
    isomin=-0.001, isomax=0.001, surface_count=1,
    colorscale=[[0, "#0aa3c2"], [1, "#0aa3c2"]], opacity=0.10,
    caps=dict(x_show=False, y_show=False, z_show=False), showscale=False))
fig.update_layout(
    template="plotly_dark", paper_bgcolor="#05030c",
    title=dict(text="THE FORBIDDEN WALL<br>"
...
Image.open(out / "_b_raw.png").resize((1600, 1000), Image.LANCZOS).save(
    out / "flying-blades.png")
