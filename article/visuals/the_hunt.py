...
H = [
    ("moments of c_t", 1, "dead"),
    ("Leibniz bound", 1, "dead"),
    ("Abel induction", 2, "dead"),
    ("monotone S", 1, "dead"),
    ("normalised X<1", 1, "dead"),
    ("factor P", 1, "dead"),
    ("PSD quadratic form", 3, "dead"),
    ("moments of A_t", 3, "dead"),
    ("orthogonal expansion", 3, "dead"),
    ("Lorentzian signature", 3, "dead"),
    ("three-term recursion", 2, "dead"),
    ("Phi >= 0 global", 2, "dead"),
    ("Phi >= 0 on support", 4, "dead"),
    ("Psi binomial >= 0", 4, "dead"),
    ("CLOSED FORM", 6, "alive"),
    ("SQUARE IDENTITY", 6, "alive"),
...
for i, (name, depth, verdict) in enumerate(H):
    fig.add_trace(
        go.Scatter3d(
            ...
            z=[0, depth],
            mode="lines",
            line=dict(color=COL[verdict], width=9),
            showlegend=False,
        )
    )
    fig.add_trace(
        go.Scatter3d(
            ...
            mode="markers",
            marker=dict(
                size=13 if verdict != "dead" else 7,
                color=COL[verdict],
    ...
    fig.add_trace(
        go.Scatter3d(
            x=[i],
            y=[0],
            z=[depth + 0.45 + 0.5 * (i % 3)],
            mode="text",
            text=[name],
            textfont=dict(color=COL[verdict], size=13 if verdict != "dead" else 10),
            showlegend=False,
        )
    )
...
fig.update_layout(
    ...
    title=dict(
        text="THE HUNT — 17 ideas tested in one day<br><sup>pink = "
    ...
    scene=dict(
        xaxis=dict(title="", showticklabels=False, color="#8f8ba0", gridcolor="#241a38"),
        ...
        camera=dict(eye=dict(x=0.15, y=-2.25, z=0.45), center=dict(x=0, y=0, z=-0.15)),
        aspectmode="manual",
        aspectratio=dict(x=2.6, y=0.35, z=0.85),
    ),
    width=1700,
    height=950,
...
Image.open(out / "_h_raw.png").resize((1700, 950), Image.LANCZOS).save(out / "the-hunt.png")
