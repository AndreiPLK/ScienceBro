...
cusps = [js[i] for i in range(1, len(js) - 1)
         if lg[i] < lg[i - 1] and lg[i] < lg[i + 1]]

fig = go.Figure()
for a, b in blocks:
    fig.add_vrect(x0=a - 0.5, x1=b + 0.5, fillcolor="#ff2a6d", opacity=0.09,
                  line_width=0)
fig.add_trace(go.Scatter(x=js, y=lg, mode="lines", line=dict(color="#6a6252",
              width=1), showlegend=False, hoverinfo="skip"))
fig.add_trace(go.Scatter(
    x=[j for j, p in zip(js, pos) if p], y=[v for v, p in zip(lg, pos) if p],
    mode="markers", name="one loop already proves it",
    marker=dict(color="#4df0ff", size=8, line=dict(color="#14130f", width=1))))
fig.add_trace(go.Scatter(
    x=[j for j, p in zip(js, pos) if not p],
    y=[v for v, p in zip(lg, pos) if not p],
    mode="markers", name="the loop dips -- needs certificates",
    marker=dict(color="#ff2a6d", size=8, line=dict(color="#14130f", width=1))))
for c in cusps:
    fig.add_annotation(x=c, y=lg[js.index(c)] - 0.9, text="cusp",
                       showarrow=False, font=dict(color="#f9f871", size=10))
...
fig.update_layout(
    ...
    xaxis=dict(title="j — which knife", color="#a8a08e", gridcolor="#322e26",
               dtick=10),
    yaxis=dict(title="log10 |depth of the minimum|", color="#a8a08e",
               gridcolor="#322e26"),
    legend=dict(font=dict(color="#a8a08e", size=11), x=0.01, y=0.06,
                bgcolor="rgba(20,19,15,0.7)"),
    margin=dict(l=80, r=30, t=132, b=70), width=1520, height=680)
...
print("saved", out, Image.open(out).size, "| cells:", len(js),
      "| blocks:", blocks, "| cusps:", cusps)
