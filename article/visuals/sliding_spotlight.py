...
Hh = np.array([sum(c * u ** (J - 1 - t) for t, c in enumerate(desc)) for u in us])
...
ann = [
    ...
    dict(
        ...
        z=1.22,
        text="POSITIVE HILL",
        font=dict(color="#8ff0ff", size=15),
        showarrow=False,
    ),
    dict(
        x=0.05,
        ...
        z=-1.05,
        text="NEGATIVE DIP",
        font=dict(color="#ff8fb0", size=15),
        showarrow=False,
    ),
]
...
fig.update_layout(
    ...
    title=dict(
        ...
        "below.</sup>",
        font=dict(color="#e8e6f0", size=23),
        x=0.025,
        y=0.965,
    ),
    scene=dict(
        xaxis=dict(
            title="u — position inside the bracket",
            backgroundcolor="#0b0714",
            gridcolor="#2a2140",
            color="#b9b4cc",
        ),
        yaxis=dict(
            title="D — dimensions of spacetime",
            backgroundcolor="#0b0714",
            gridcolor="#2a2140",
            color="#b9b4cc",
        ),
        zaxis=dict(
            title="what gets integrated",
        ...
        annotations=ann,
        camera=dict(eye=dict(x=1.45, y=-1.55, z=0.72), center=dict(x=0, y=0, z=-0.06)),
    ),
...
print(
    ...
    round(TH, 2),
    "| flips at D =",
    None if flip is None else round(flip, 2),
)
