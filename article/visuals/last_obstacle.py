...
for j in range(5, 19):
    ...
    for fr in frs:
        v = vmax * fr
        val = sum(((-1) ** t) * float(e[t]) * M1f(j, n, t) * v**t for t in range(min(j, len(e))))
...
ann = [
    dict(
        ...
        font=dict(color="#3fe7f5", size=15),
        showarrow=False,
    ),
    dict(
        x=0.90,
        ...
        font=dict(color="#ff5f95", size=14),
        showarrow=False,
    ),
    dict(
        x=0.06,
...
fig.update_layout(
    ...
    title=dict(
        text="THE LAST OBSTACLE — one picture, one problem left<br>"
    ...
    scene=dict(
        xaxis=dict(
            title="across the support of the measure  ->  edge",
            color="#8f8ba0",
            gridcolor="#241a38",
        ),
        yaxis=dict(title="knife j", color="#8f8ba0", gridcolor="#241a38", dtick=2),
        zaxis=dict(title="", showticklabels=False, color="#8f8ba0", gridcolor="#241a38"),
        annotations=ann,
        camera=dict(eye=dict(x=1.55, y=-1.55, z=0.62), center=dict(x=0, y=0, z=-0.08)),
    ),
    width=1600,
    height=1000,
...
Image.open(out / "_lo_raw.png").resize((1600, 1000), Image.LANCZOS).save(out / "last-obstacle.png")
