...
fig = make_subplots(
    ...
    subplot_titles=("every overtone of one polynomial", "the coefficients at three heights"),
)

fig.add_trace(
    go.Heatmap(
        x=MS,
        y=[float(d) for d in DS],
        z=Zc,
        zmid=0,
        colorscale=[
            [0.0, "#7a1f45"],
        ...
        colorbar=dict(
            title=dict(
                text="sign, and size relative<br>to that row's largest",
                font=dict(color="#a8a08e", size=10),
            ),
            tickfont=dict(color="#a8a08e", size=9),
            thickness=12,
            len=0.78,
            x=0.56,
        ),
...
fig.add_hline(y=SHORE, line=dict(color="#f9f871", width=2, dash="dash"), row=1, col=1)
fig.add_annotation(
    x=MS[len(MS) // 2],
    y=SHORE + 1.6,
    text=f"THE SHORE  D = {SHORE:.2f}",
    showarrow=False,
    font=dict(color="#f9f871", size=13),
    row=1,
    col=1,
)
...
for D, col, name in (
    (F(6), "#4df0ff", "D = 6, far below the shore"),
    ...
    rel = [np.sign(y) * max(0.02, 1 + np.log10(abs(y) / top) / 8) if y else 0.0 for y in ys]
    fig.add_trace(
        go.Scatter(
            x=MS,
            y=rel,
...
fig.update_layout(
    ...
    xaxis=dict(
        title="m = n - j   (large m = low spin)", color="#a8a08e", gridcolor="#322e26", dtick=2
    ),
    yaxis=dict(title="D — spacetime dimension", color="#a8a08e", gridcolor="#322e26"),
    xaxis2=dict(title="m = n - j", color="#a8a08e", gridcolor="#322e26", dtick=2),
    yaxis2=dict(
        title="sign x relative size (row-normalised)", color="#a8a08e", gridcolor="#322e26"
    ),
    legend=dict(
        font=dict(color="#a8a08e", size=10),
        x=0.63,
    ...
    margin=dict(l=80, r=30, t=132, b=118),
    width=1560,
    height=700,
)
...
print("saved", out, Image.open(out).size, "| cells:", Zc.size, "| shore:", round(SHORE, 3))
