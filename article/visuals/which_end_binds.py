...
DS = [F(3), F(4), F(5), F(6), F(8), F(11), F(16), F(23), F(32), F(40), F(60), F(90), F(120), F(180)]
LEVELS = [12, 24, 40]
        ...
        for lam in LAMS:
            ...
            for D in DS:
                if float(D) >= sh:
                    ...
                    continue
                vals = [
                    float(F(-1) ** m * jacobi_coeff_fast(n - m, n, lam, D)) for m in range(n - 1)
                ]
                top = max(vals)
            ...
            grid.append(row)
            print(f"  n={n} lam={lam} done", flush=True)
        out[str(n)] = grid
...
for col, n in enumerate(LEVELS, start=1):
    ...
    fig.add_trace(
        go.Heatmap(
            x=[float(x) for x in DS],
            y=[float(x) for x in LAMS],
            z=Z,
            zmin=0,
            zmax=2,
            showscale=(col == len(LEVELS)),
            colorscale=[
                [0.0, "#ff2a6d"],
                [0.33, "#ff2a6d"],
                [0.34, "#f9f871"],
            ...
            colorbar=dict(
                ...
                ticktext=["lowest spin binds", "an interior spin", "leading trajectory binds"],
...
fig.update_layout(
    ...
    margin=dict(l=70, r=30, t=132, b=70),
    width=1620,
    height=620,
)
for i in range(1, len(LEVELS) + 1):
    fig.update_xaxes(title="D", type="log", color="#a8a08e", gridcolor="#322e26", row=1, col=i)
    fig.update_yaxes(
        title="lam" if i == 1 else None,
        type="log",
        color="#a8a08e",
        gridcolor="#322e26",
        row=1,
