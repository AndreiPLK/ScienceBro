...
def probe_points() -> list[np.ndarray]:
    ...
    combos = [
        ...
        (0.05, 0.55),
        (0.10, 0.45),
        (0.15, 0.35),  # exterior side
        (0.30, 0.28),
        (0.40, 0.22),
        (0.45, 0.30),
        (0.50, 0.25),  # near the diagonal
        (0.55, 0.20),
        (0.60, 0.10),
        (0.65, 0.05),
        (0.55, 0.35),  # interior side
    ]
...
def evaluate(model: str | None) -> list[dict]:
    ...
    for p in pts:
        row: dict = {
            ...
            "region_analytic": "interior"
            if abs(np.cos(2 * p[0]) / np.cos(2 * p[1])) < 1.0
            else "exterior",
        }
        try:
...
def main() -> int:
    ...
    def summarise(rows: list[dict]) -> str:
        ...
        return (
            f"{len(tr)} future-trapped, {len(at)} anti-trapped, {len(un)} untrapped (of {len(ok)})"
        )
