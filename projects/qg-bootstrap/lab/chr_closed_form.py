...
def P_closed(j, n, lam, D):
    ...
    s = lam + n - 1
    R = (D + c) / 2 + j - 1
    c0 = (Fraction(factorial(2 * n - 2), factorial(j - 1) * 2 ** (j - 1))
          * s ** (2 * (j - 1)))
    tot = Fraction(0)
    for t in range(j):
        num = poch(Fraction(1) - j, t) * poch(1 - R, t)
        den = (poch(Fraction(1) - n, t) * poch(Fraction(3, 2) - n, t)
               * s ** (2 * t))
...
def main() -> int:
    ...
    for j in range(2, 26):
        for n in range(j + 1, j + 26, 3):
            for lam in (Fraction(1, 100), Fraction(1, 2), Fraction(3),
                        Fraction(26), Fraction(150)):
                Th = T_hat(lam)
                for f in (Fraction(0), Fraction(1, 2), Fraction(1),
                          Fraction(3)):          # incl. outside the shore
                    D = 4 + (Th - 4) * f
                    ...
                    checks += 1
                    if got != reff:
                        mismatches.append({"j": j, "n": n, "lam": str(lam),
                                           "D": str(D)})
    out = {"identity": "P_j = c0 * sum_t (-1)^t E_2t (1-j)_t (1-R)_t /"
                       " ((1-n)_t (3/2-n)_t s^{2t})",
           "definitions": {"s": "lam + n - 1", "c": "4n - 4j - 1",
                           "R": "(D + c)/2 + j - 1",
                           "c0": "(2n-2)!/((j-1)! 2^{j-1}) * s^{2(j-1)} > 0"},
           "exact_checks": checks, "mismatches": mismatches,
           "verified": not mismatches,
           "significance": ("terminating hypergeometric structure; all "
    ...
    (RES / "chr_closed_form.json").write_text(json.dumps(out, indent=1),
                                              encoding="utf-8")
    print(f"exact checks: {checks}, mismatches: {len(mismatches)}", flush=True)
    print("CLOSED FORM " + ("VERIFIED" if not mismatches else "FAILED"),
          flush=True)
