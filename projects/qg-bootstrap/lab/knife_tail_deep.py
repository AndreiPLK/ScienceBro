...
def bisect_t(builder, a, b, depth, tag, log):
    ...
    if depth == 0:
        return False
    mid = (a + b) / 2
    return bisect_t(builder, a, mid, depth - 1, tag, log) and bisect_t(
        builder, mid, b, depth - 1, tag, log
    )
...
def main() -> int:
    ...
    for kk in [] if STAGE == "belowdiag" else range(3, 46):
        ...
        stail_ok &= got
        print(
            f"shallow-tail k={kk}: {'OK' if got else 'FAIL'} ({time.time() - t0:.0f}s)", flush=True
        )
    ...
    for mv in [] if STAGE == "belowdiag" else range(max(1, J - 2), 41):
        ...
        for i2 in range(dh + 1):
            b = sum(sp.binomial(i2, q) / sp.binomial(dh, q) * cs[q] for q in range(i2 + 1))
    ...
    lam_e = (K + 45 + thL) * r3 / 3
    kk_ = K + 45 + 2
    Tk = 3 * (2 * kk_ - 3) / (kk_ * (kk_ - 2)) * (lam_e**2 + (2 * kk_ - 2) * lam_e + 1) + 2 * kk_
    ...
    def deep_cell(a1, b1, a2, b2, depth):
        ...
        m2 = (a2 + b2) / 2
        return (
            deep_cell(a1, m1, a2, m2, depth - 1)
            and deep_cell(a1, m1, m2, b2, depth - 1)
            and deep_cell(m1, b1, a2, m2, depth - 1)
            and deep_cell(m1, b1, m2, b2, depth - 1)
        )
    ...
    def bern1(expr, var, elev=None):
        ...
        cs = [pp.coeff_monomial(var**q) if q <= pp.degree() else 0 for q in range(d + 1)]
        # коэффициенты полинома в мономах остаются прежними; Бернштейн
        # берём на поднятой степени d
        cs = [
            pp.coeff_monomial(var**q) if q <= pp.degree() else sp.Integer(0) for q in range(d + 1)
        ]
        out = []
        for i2 in range(d + 1):
            b = sum(
                sp.binomial(i2, q) / sp.binomial(d, q) * cs[q]
    ...
    def deep_cell3(vsub, extra_var, a1, b1, a2, b2, depth, tag):
        ...
        for e in exprs:
            if extra_var is sig:
                ...
                subs_ok = all(
                    (cc >= 0) if not cc.has(r3) else (cc.coeff(r3, 0) >= 0 and cc.coeff(r3, 1) >= 0)
                    for _, cc in Pp.terms()
                )
            if not subs_ok:
                allok = False
                break
        log.append({"cell": tag, "box": [str(a1), str(b1), str(a2), str(b2)], "ok": bool(allok)})
        if allok:
            return True
        if depth == 0:
            return False
        ...
        m2 = (a2 + b2) / 2
        return all(
            deep_cell3(vsub, extra_var, aa, bb, cc2, dd, depth - 1, tag)
            for aa, bb, cc2, dd in [
                (a1, m1, a2, m2),
                (a1, m1, m2, b2),
                (m1, b1, a2, m2),
    ...
    def deep_cell4(vsub, extra_var, a1, b1, depth, tag):
        ...
        P = P_sym(
            lam_c.subs(K, K + 8) if "Ktail" in tag else lam_c,
            (Tk_c.subs(K, K + 8) if "Ktail" in tag else Tk_c) - yv,
            41 + vsub,
        )
        P = sp.expand(sp.fraction(sp.together(P))[0])
            ...
            for b_ in bern1(e, thL):
                if extra_var is sig:
                    ...
                    okc = all(
                        (cc >= 0)
                        if not cc.has(r3)
                        else (cc.coeff(r3, 0) >= 0 and cc.coeff(r3, 1) >= 0)
                        for _, cc in Pp.terms()
                    )
                if not okc:
                    allok = False
        ...
        if depth == 0:
            return False
        mid = (a1 + b1) / 2
        return deep_cell4(vsub, extra_var, a1, mid, depth - 1, tag) and deep_cell4(
            vsub, extra_var, mid, b1, depth - 1, tag
        )

    # v5: K-расщепление: K=0..7 явно (2-вар), хвост K>=8 отдельно
    gotA = True
    for K0 in range(0, 8):
        got_k = deep_cell4((K + 4) * sig, sig, sp.Integer(0), sp.Integer(1), 6, f"bd_K{K0}")
    ...
    def deep_cell4_atK(K0, a1, b1, depth, tag):
        ...
        if depth == 0:
            return False
        mid = (a1 + b1) / 2
        return deep_cell4_atK(K0, a1, mid, depth - 1, tag) and deep_cell4_atK(
            K0, mid, b1, depth - 1, tag
        )
    ...
    for K0 in range(0, 8):
        ...
        print(
            f"  below-diag K={K0}: {'OK' if gk else 'FAIL'} ({time.time() - t0:.0f}s)", flush=True
        )
        gotA &= gk
    # хвост K >= 8
    gk = deep_cell4((K + 8 + 4) * sig, sig, sp.Integer(0), sp.Integer(1), 6, "bd_Ktail")
    print(f"  below-diag K>=8: {'OK' if gk else 'FAIL'} ({time.time() - t0:.0f}s)", flush=True)
    gotA &= gk
    print(
        f"deep-tail below-diagonal: {'OK' if gotA else 'FAIL'} ({time.time() - t0:.0f}s)",
        flush=True,
    )
    gotB = (
        True
    ...
    got = gotA and gotB
    ok &= got
    print(f"deep-tail: {'OK' if got else 'FAIL'} ({time.time() - t0:.0f}s)", flush=True)
    ...
    out = {
        ...
        "cells": len(log),
        "stage_verdicts": {
            "shallow_tail": (None if STAGE == "belowdiag" else bool(stail_ok)),
            "deep_fixed": (None if STAGE == "belowdiag" else bool(deep_fixed_ok)),
            "deep_tail_below_diag": bool(gotA),
            "deep_tail_above_diag": (None if STAGE == "belowdiag" else bool(gotB)),
        },
        "failed_cells": failed_cells[:50],
        "n_failed": len(failed_cells),
        "env": {"KNIFE_STAGE": STAGE, "BERN_ELEV": os.environ.get("BERN_ELEV", "0")},
        "scope_note": "stage_verdicts null = stage NOT RUN in this "
        "invocation; all_certified covers only stages run",
        "command": f"KNIFE_J={J} KNIFE_STAGE={STAGE} python lab/knife_tail_deep.py",
        "git": git,
        "runtime_s": round(time.time() - t0, 1),
    }
    (RES / f"knife_tail_deep_j{J}.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
