...
def main() -> int:
        ...
        for n in range(max(4, j + 1), max(4, j + 1) + 30, 2):
            for lam in LAMS:
                ...
                if sc > 1 and len(many) < 25:
                    many.append({"j": j, "n": n, "lam": str(lam), "sign_changes": sc})
        if j % 5 == 0 or j < 6:
            print(
                f"  j={j}: cells {cells}, histogram "
    ...
    ok = set(hist) <= {0, 1}
    out = {
        "question": "number of sign changes of the D-free shape Hhat on"
        ...
        "grid": {"j": "2..60", "n": "max(4,j+1)..+30 step 2", "lam": [str(x) for x in LAMS]},
        "cells": cells,
        "sign_change_histogram": {str(k): v for k, v in sorted(hist.items())},
    ...
    (RES / "keystone_descartes.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
