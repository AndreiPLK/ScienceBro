...
def validate(j: int, n: int, k: int) -> list[str]:
    ...
    for fv in (Fraction(1, 3), Fraction(1), Fraction(3)):
        ...
        for fw in (Fraction(1, 5), Fraction(2), Fraction(9)):
            ...
            if (nv > 0) != (jv > 0):
                bad.append(f"j={j} n={n} k={k} lam={lam} w={fw}: "
                           f"N>0={nv > 0} but J>0={jv > 0}")
    return bad
...
def main() -> int:
    ...
    if problems:
        out = {"validation_failed": problems, "certified": 0,
               "command": "python lab/keystone_symbolic.py", **stamp()}
        (RES / "keystone_symbolic.json").write_text(json.dumps(out, indent=1),
                                                    encoding="utf-8")
        ...
        for n in range(max(4, j + 1), max(4, j + 1) + N_SPAN, 2):
            for k in range(3, K_MAX + 1):
                ...
                if ok:
                    ...
                    failures.append({"j": j, "n": n, "k": k,
                                     "lam_range": [str(a), str(b)],
                                     "depth_reached": d})
    ...
    out = {"claim": "J(Q) > 0 for EVERY lam on each shore branch and every D"
           ...
           "uniform_clearing": "every term carries exactly (1+v)^{2(j-1)}"
                               " and (1+w)^{j-1}",
           "coverage": {"j": f"{J_MIN}..{J_MAX}",
                        "n": f"max(4,j+1)..+{N_SPAN} step 2",
                        "branches": f"k=3..{K_MAX}, i.e. lam in (0, "
                                    f"{float(branch_range(K_MAX)[1]):.2f}]",
                        "NOT covered": "lam above the last branch; needs the"
                                       " tail argument, as in the knife"
                                       " theorems",
                        "j=2 excluded_on_purpose":
                            "at j=2 the fleet is EXACTLY tangent to the shore"
    ...
    print("SYMBOLIC-LAM CERTIFICATE " + ("HOLDS" if certified == cells
