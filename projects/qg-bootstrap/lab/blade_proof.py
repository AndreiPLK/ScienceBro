...
def Tk_of(k_expr):
    return (
        3 * (2 * k_expr - 3) / (k_expr * (k_expr - 2)) * (lam**2 + (2 * k_expr - 2) * lam + 1)
        + 2 * k_expr
    )
...
def cell_fixed_m(Bk, Vk, disc, mv, lam_sub, log, tag):
    ...
    roots = sorted([sp.nsimplify(r) for r in sp.Poly(d, t).real_roots() if 0 <= r <= 1])
...
def certify_cell(expr, mvar, kapvar, a, b, depth, log, tag):
    ...
    if depth == 0:
        ...
        return False
    mid = (a + b) / 2
    return certify_cell(expr, mvar, kapvar, a, mid, depth - 1, log, tag) and certify_cell(
        expr, mvar, kapvar, mid, b, depth - 1, log, tag
    )
...
def main() -> int:
    ...
    for name, E in (("k3_B", B3), ("k3_V", V3)):
        ...
        got = certify_cell(sp.expand(e * 81), v, None, sp.Integer(0), sp.Integer(1), 4, log, name)
        ok &= got
    ...
    for kk in range(4, KMAX + 1):
        ...
        while m_start <= 120 and not tail_done:
            for m0 in range(1 if m_start == 4 else m_start - 6, m_start):
                branch_ok &= cell_fixed_m(Bk, Vk, disc, m0, lam_sub, log, f"k{kk}_m{m0}")
            ...
            for name, E in ((f"k{kk}_Btail", Bk), (f"k{kk}_Vtail", Vk)):
                ...
                e = sp.expand(sp.fraction(sp.together(e))[0])
                got = certify_cell(
                    e, v, None, sp.Integer(0), sp.Integer(1), 3, [], f"{name}_m{m_start}"
                )
                if name.endswith("Btail"):
                    ...
                    okV = got
            if okB and okV:
                log.append({"cell": f"k{kk}_tail", "m_start": m_start, "cert": "OK"})
                tail_done = True
            else:
                m_start += 6
        branch_ok &= tail_done
    ...
    def cert_q3(E, gens_and_ranges, tag):
        ...
        for gvar in gens_and_ranges:
            ...
            num = sp.expand(
                sp.fraction(sp.together(num.subs(gvar, w / (1 + w)) * (1 + w) ** deg))[0]
            )
        P = sp.Poly(num, v, w)
    ...
    okL2 = all(c >= 0 for _, c in sp.Poly(e1, v).terms()) and all(
        c >= 0 for _, c in sp.Poly(e2, v).terms()
    )
    ok &= okL2
    ...
    okL3 = cert_q3(Bd, [z], "tail_L3a_B_at_asymptote") and cert_q3(Vd, [z], "tail_L3b_vertex")
    ok &= okL3
    print(
        f"TAIL lam>=26: T0={not t0fails} L2={okL2} L1={okL1} L3={okL3} ({time.time() - t0:.0f}s)",
        flush=True,
    )
    ...
    out = {
        "theorem": "j=3 blades never cut the shore - COMPLETE: branches k=3..45 (lam<=26.1) + tail lam>=26 (T0/L2/L1/L3)",
        "branches": "k=3 on (0,2/3]; k=4 on [2/3,3/2]; k=5..45 on [3(2k-5)/10, 3(2k-3)/10]; tail lam>=26",
        "all_certified": bool(ok),
        "cells": log,
        "command": "python lab/blade_proof.py",
        "git": git,
        "runtime_s": round(time.time() - t0, 1),
    }
    (RES / "blade_proof.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
