...
def hat_bracket(n):
    mu = (n + lam - 1) / lam
    t = -mu * (1 - x) / 2
    Q = sp.prod([sp.Rational(1, 2) * (1 + lam) + lam * t + lam * (k - 1) for k in range(1, n)])
    P = sp.expand(Q**2)
...
def main() -> int:
    ...
    for Dv in range(10, 41, 2):
        ...
        for n, S in brackets.items():
            ...
            reals = sorted(
                [sp.nsimplify(r) for r in roots if sp.im(r) == 0], key=lambda r: float(r)
            )
        ...
        if intervals:
            ...
            lo = hi = None
        out.append({"D": Dv, "excluded_union_approx": [lo, hi], "per_level": intervals})
        print(
            f"D={Dv}: excluded lambda union ~ [{lo}, {hi}] ({len(intervals)} active levels)",
            flush=True,
        )
    (RES / "grav_lambda_map.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
