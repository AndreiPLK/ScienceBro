...
def lin_n(const: Fraction, n_coef: Fraction, n0: int):
    ...
    return CTX.from_dict({(0, 0, 0): q(const + n_coef * n0),
                          (0, 0, 1): q(2 * n_coef)})
...
def main() -> int:
    ...
    for (j, parity, k) in ((3, 0, 5), (4, 1, 3), (5, 0, 8), (6, 1, 6)):
        ...
        fast_map = {tuple(fast.monomial(i)): fast.coefficient(i)
                    for i in range(len(fast))}
        ref_map = {ky: Fraction(int(c.p), int(c.q))
                   for ky, c in ref.c.items() if c != 0}
        same = (len(fast_map) == len(ref_map)
                and all(Fraction(int(fast_map[ky].p), int(fast_map[ky].q))
                        == ref_map[ky] for ky in ref_map if ky in fast_map))
        if not same or cert_ok(fast) != slow.cert_ok(ref):
            mism.append({"j": j, "parity": parity, "k": k,
                         "fast_terms": len(fast_map),
                         "ref_terms": len(ref_map)})
    if mism:
        out = {"validation_failed": mism, "command":
               "python lab/keystone_strip_fast.py", **stamp()}
        (RES / "keystone_strip_fast.json").write_text(
            json.dumps(out, indent=1), encoding="utf-8")
        print("VALIDATION FAILED vs QPoly engine:", mism, flush=True)
        return 2
    print("validation contract: OK (identical coefficients to QPoly engine)",
          flush=True)
    ...
    for j in (6, 8):
        ...
        t_slow = time.time() - t2
        bench.append({"j": j, "fast_s": round(t_fast, 4),
                      "qpoly_s": round(t_slow, 4),
                      "speedup": round(t_slow / t_fast, 1) if t_fast else None})
        print(f"  j={j}: native {t_fast:.3f}s vs QPoly {t_slow:.3f}s "
              f"-> {t_slow/t_fast:.1f}x", flush=True)
    ...
    out = {"engine": "native flint fmpq_mpoly",
           ...
           "benchmark": bench,
           "coverage": {"j": f"{J_MIN}..{J_MAX}", "branches": f"3..{K_MAX}",
    ...
    (RES / "keystone_strip_fast.json").write_text(json.dumps(out, indent=1),
                                                  encoding="utf-8")
