...
def interpolate(pts: list[tuple[Fraction, Fraction]]) -> list[Fraction]:
    ...
    for k in range(1, len(xs)):
        prev = dd[-1]
        dd.append([(prev[i + 1] - prev[i]) / (xs[i + k] - xs[i])
...
def main() -> int:
    ...
    for t in range(1, 9):
        for parity, label in ((0, "even"), (1, "odd")):
            ...
            fact_a.append({"E_2t": 2 * t, "parity": label,
                           "fitted_on": fit_k,
                           "degree": len(pol) - 1,
    ...
    out = {"purpose": "show that n can be carried symbolically at fixed j,"
                      " which is what step 2 of the keystone needs",
           "fact_A": {"statement": "for fixed t and fixed parity of n,"
                                   " E_2t(n) is a polynomial in n",
                      "method": "fit on the first points, PREDICT the rest",
                      "rows": fact_a, "verified": a_ok},
           "fact_B": {"statement": "Q_n(t)/Q_n(0) = prod_{i=1..t} (j-i)/(n-i)"
                                   " -- the factorial growth is an overall"
                                   " positive constant, the t-dependence is"
                                   " rational in n of degree t",
                      "checks": fact_b_checks, "failures": fact_b_bad,
    ...
    (RES / "keystone_npoly.json").write_text(json.dumps(out, indent=1),
                                             encoding="utf-8")
    print(f"Fact A (E_2t polynomial in n): "
          f"{'VERIFIED' if a_ok else 'FAILED'} "
          f"({len(fact_a)} cases, each predicting held-out points)",
          flush=True)
    print(f"Fact B (weight is a constant times a rational function): "
          f"{fact_b_checks} checks, {len(fact_b_bad)} failures", flush=True)
