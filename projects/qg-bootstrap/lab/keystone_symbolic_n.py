"""KEYSTONE STEP 2: carry the LEVEL n symbolically as well.

Step 1 (lab/keystone_symbolic.py) made lam symbolic and closed j = 3..20 on
7740 cells. This module adds the level: at fixed j and fixed parity, n
becomes a variable too, so one certificate covers ALL levels at once.

Why that is possible (verified in results/keystone_npoly.json):
  FACT A  E_2t(n) is a polynomial in n for fixed t and fixed parity.
  FACT B  Q_n(t)/Q_n(0) = prod_{i=1..t} (j-i)/(n-i): the factorial growth is
          an overall POSITIVE constant and divides out of any sign question.

Substitutions (three rays, all mapped to the closed first orthant):
        n   = n0 + 2 mu,                      mu >= 0   (parity fixed)
        lam = (a + b v)/(1 + v),              v  >= 0   (shore branch k)
        Q   = (n - j) + Delta(lam) w/(1+w),   w  >= 0   (D from 4 to shore)

UNIFORM CLEARING (this is where the far-below sign bug came from, so it is
spelled out): multiply the whole sum by

        (1+w)^{j-1} * (1+v)^{2(j-1)} * prod_{i=1}^{j-1}(n-i) * s^{2(j-1)} ,

all four factors strictly positive on the domain (n > j-1 makes the third
one positive). Term t then carries exactly:
  * t factors [ (2n-2j+3/2+i)(1+w)(1+v)^2 + Delta_num w ]   (i = m..j-2)
  * (1+w)^{j-1-t}                                    -> total (1+w)^{j-1}
  * s_num^{2(j-1-t)}                                 -> total (1+v)^{2(j-1)}
  * prod_{i=t+1}^{j-1}(n-i)   (its own denominator prod_{i=1}^{t}(n-i) is
    cancelled by the common factor)
  * alpha_t = prod_{i=0}^{m-1}(p+1+i), p = n-j-1/2,  m = j-1-t
so every term has the SAME total degree in (1+w) and (1+v), and the sum is a
single polynomial N(w, v, mu) whose sign equals the sign of the bracket.

CERTIFICATE: all coefficients of N nonnegative with a positive constant term
=> N > 0 on the closed orthant => the knife never cuts below the shore, for
EVERY level n of that parity, EVERY lam on that branch, and EVERY D from 4
up to the shore.

VALIDATION CONTRACT (runs first, verdict withheld if it fails): the sign of
N at interior rational points must match the exact rational J from
keystone_beta.py at the corresponding (n, lam, D).

Run: KSN_JMAX=6 python lab/keystone_symbolic_n.py
Artifact: results/keystone_symbolic_n.json
"""
...
def E2t_in_n(t: int, parity: int) -> list[Fraction]:
    ...
    fit_k = 3 * t + 5
    pts = [(Fraction(n), Fraction(e_doubled_int(n)[t]))
...
def linear_in_n(const: Fraction, n_coef: Fraction, n0: int) -> QPoly:
    ...
    return qp({(0, 0, 0): const + n_coef * n0,
               (0, 0, 1): 2 * n_coef})
...
def build_N(j: int, parity: int, k: int, n0: int) -> QPoly:
    ...
    s_num = (linear_in_n(a - 1, Fraction(1), n0)
             + linear_in_n(b - 1, Fraction(1), n0) * v_p)
    ...
    for t in range(j):
        ...
        for i in range(m, j - 1):                   # the Q-factors
            coef = linear_in_n(Fraction(3, 2) - 2 * j + i, Fraction(2), n0)
            term = term * (coef * (onepw * onepv2) + delta_num * w_p)
...
def cert_ok(N: QPoly) -> bool:
    ...
    return bool(vals) and all(v >= 0 for v in vals) and any(v > 0
                                                            for v in vals)


def main() -> int:
    t0 = time.time()
    problems = (validate(3, 0, 5, 6) + validate(4, 1, 3, 7)
                + validate(5, 0, 8, 8))
    if problems:
        out = {"validation_failed": problems[:10], "certified": 0,
               "command": "python lab/keystone_symbolic_n.py", **stamp()}
        (RES / "keystone_symbolic_n.json").write_text(
    ...
    for j in range(3, J_MAX + 1):
        for parity in (0, 1):
            ...
            for k in range(3, K_MAX + 1):
                ...
                if gap:
                    failures.append({"j": j, "parity": parity, "k": k,
                                     "reason": "base level outside step-1"
                                               " coverage", "levels": gap})
                    continue
                certified += 1
                rows.append({"j": j, "parity": parity, "k": k,
                             "n_tail_start": n_tail,
    ...
    out = {"claim": "one certificate per (j, parity, branch) covering ALL"
                    " levels n of that parity, ALL lam on the branch, and ALL"
                    " D from 4 up to the shore",
           "substitutions": {"n": "n0 + 2 mu, mu >= 0",
                             "lam": "(a + b v)/(1+v), v >= 0",
           ...
           "coverage": {"j": f"3..{J_MAX}", "branches": f"k=3..{K_MAX}",
                        "n": "every level of the stated parity, unbounded",
                        "NOT covered": "j above the tested range, and lam"
           ...
           "max_shift_M": max_M, "max_tail_start_level": max_tail,
           "rows": rows[:60],
           "failures": failures,
           "command": f"KSN_JMAX={J_MAX} python lab/keystone_symbolic_n.py",
           **stamp(), "runtime_s": round(time.time() - t0, 1)}
    (RES / "keystone_symbolic_n.json").write_text(json.dumps(out, indent=1),
                                                  encoding="utf-8")
    print(f"cells {cells}, certified {certified}", flush=True)
    print("SYMBOLIC-n CERTIFICATE " + ("HOLDS" if certified == cells
