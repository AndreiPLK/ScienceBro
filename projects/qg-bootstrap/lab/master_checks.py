...
def master_bracket(n: int, j: int):
    ...
    B = sp.Integer(0)
    for i in range(j):
        w = sp.Integer(factorial(2 * n - 2 * j + 2 * i)) / (factorial(i) * 2**i)
...
def main() -> int:
    ...
    for n in range(3, 12):
        ...
        ref = sp.expand(3 * (2 * n - 3) * s**2 - n * (n - 2) * (D + 4 * n - 9))
    ...
    (RES / "T2n10_brackets.json").write_text(
        json.dumps(
            {
                "6": {"numerator_factored": str(fac), "denominator_factored": str(den)},
    ...
    over_ok = len(mono) == 15 and all(sp.simplify(mono[k] - scale * mp[k]) == 0 for k in mono)
    out["monomial_overdetermination"] = {
        "monomials": len(mono),
        "dof": 5,
        "all_match": bool(over_ok),
    }
    ...
    for n, j in cases:
        for Dv in range(4, 37, 4):
            for lamv in lams:
                grid_checks += 1
                se = (lambda v: (v > 0) - (v < 0))(a_l(n, 2 * n - 2 * j, lamv, Dv))
                if se != master_sign(n, j, lamv, Dv):
                    grid_mism += 1
    out["sign_grid"] = {"checks": grid_checks, "mismatches": grid_mism, "blind_j6_cases": 4}

    git = subprocess.run(
        ["git", "rev-parse", "--short", "HEAD"], capture_output=True, text=True
    ).stdout.strip()
    out["_meta"] = {
        "command": "python lab/master_checks.py",
        "git": git,
        "symbolic_matches": n_sym_matches,
        "runtime_s": round(time.time() - t0, 1),
    }
    (RES / "master_checks.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
    ok = not out["fails"] and over_ok and grid_mism == 0
    print(
        f"symbolic matches: {n_sym_matches}/{n_sym_matches + len(out['fails'])};"
        f" overdetermination 15/15: {over_ok};"
        f" sign grid: {grid_checks - grid_mism}/{grid_checks}",
        flush=True,
    )
