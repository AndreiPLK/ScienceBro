...
def cell_certify(coeffs, j, kk, a, b, onto_zero=False):
    ...
    onepw2 = onepw * onepw
    tk_num = r * (numlam * numlam + (2 * kk - 2) * (numlam * onepw) + onepw2) + (2 * kk) * onepw2
    ...
    for name, basep, store, top in (
        (0, numlam, lam_pows, dmax),
        (1, numd, d_pows, qmax),
        (2, onepw, w_pows, dmax),
        (3, onepu, u_pows, qmax),
    ):
...
def cell_shallow(coeffs, j, kk, mu_lo, mu_hi, depth, log, tag, onto_zero=False):
    def try_cell(a, b, d):
...
def main() -> int:
    ...
    log = []
    ok = True
    bj_cache = {mv: Bj_coeffs(J, mv) for mv in range(j_min_m(J), MFIX + 1)}
    for kk in range(3, 46):
        ...
        for mv in range(j_min_m(J), MFIX + 1):
            got = cell_shallow(
                bj_cache[mv], J, kk, mu_lo, mu_hi, 4, log, f"j{J}_k{kk}_m{mv}", onto_zero=onto
            )
            branch_ok &= got
            if not got:
    ...
    (RES / f"knife_proof2_j{J}.json").write_text(json.dumps(out, indent=1), encoding="utf-8")
