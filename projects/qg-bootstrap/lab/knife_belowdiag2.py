...
assert _kt2.J == J, (
    f"J mismatch: belowdiag2 J={J} but knife_tail2.build_P uses J={_kt2.J}; set KNIFE_J for both"
)
...
def cert(P, has_K, tag, log, t0, strict_needed=True, strict_ok=None):
    ...
    for kl, (ad, bd) in collect_pairs(A, B, (0, 1, 2)).items():
        ...
        if strict_needed and _sq3(afl.get(0, fmpq(0)), bfl.get(0, fmpq(0))) <= 0:
            strict_ok[0] = False
...
def bisect(builder, a, b, depth, has_K, tag, log, t0, strict_ok=None):
    ...
    if depth == 0:
        return False
    mid = (a + b) / 2
    return bisect(builder, a, mid, depth - 1, has_K, tag, log, t0, strict_ok) and bisect(
        builder, mid, b, depth - 1, has_K, tag, log, t0, strict_ok
    )
...
def main() -> int:
    ...
    got = bisect(
        lambda a, b: build(K2, K2 + 10 * sig, a, b),
        Fraction(0),
        Fraction(1),
        5,
        True,
    ...
    for K0 in range(0, 6):
        got = bisect(
            lambda a, b, K0=K0: build(Q3Poly.const(NV, K0 - 6), (K0 + 4) * sig, a, b),
            Fraction(0),
            Fraction(1),
            5,
            False,
