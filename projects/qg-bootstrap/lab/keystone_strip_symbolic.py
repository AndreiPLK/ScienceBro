...
def build_N(j: int, parity: int, k: int, n0: int) -> QPoly:
    ...
    for t in range(j):
        ...
        for i in range(m, j - 1):                   # the STRIP Q-factors
            ...
            fac = ((q(Fraction(1, 2)) * t_num + base * onepv2) * onepw
                   + w_p * onepv2)
            term = term * fac
...
def main() -> int:
    ...
    if problems:
        out = {"validation_failed": problems[:10], "certified": 0,
               "command": "python lab/keystone_strip_symbolic.py", **stamp()}
    ...
    (RES / "keystone_strip_symbolic.json").write_text(json.dumps(out, indent=1),
                                                  encoding="utf-8")
