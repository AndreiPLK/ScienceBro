            ...
            for D in DS:
                ...
                for m in range(n - 1):
                    if sign_of(F(-1) ** m * jacobi_moment(n - m, n, lam, D)) <= 0:
                        neg.append(m)
                rows[key] = {"n": n, "lam": str(lam), "D": str(D),
                             "knives": n - 1, "negative_m": neg,
                             "shore": round(shore, 4),
                ...
                print("  n=%3d lam=%-4s D=%-4s knives %3d  negatives %s  (%.1fs)"
                      % (n, lam, D, n - 1, neg if neg else "none",
                         time.time() - t1), flush=True)
    print("TOTAL knives certified: %d, failures: %d" % (knives, failures),
          flush=True)
    return 0
