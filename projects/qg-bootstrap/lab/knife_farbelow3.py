...
def build(a, b):
    ...
    kk = Kc + 53
    den = kk * (kk - 2)
    tk_num = ((3 * (2 * kk - 3)) * (lam * lam + (2 * kk - 2) * lam + 1)
              + (2 * kk) * kk * (kk - 2))
...
def bisect(a, b, depth, log, strict_flag):
    ...
    if depth == 0:
        return False
    mid = (a + b) / 2
    return (bisect(a, mid, depth - 1, log, strict_flag)
            and bisect(mid, b, depth - 1, log, strict_flag))
...
def main() -> int:
    ...
    print(f"FAR-BELOW-INTERVAL(j={J}) "
          + ("CLOSED" if ok else "OPEN")
          + f"  cells={len(log)} ({time.time()-t0:.0f}s)", flush=True)
