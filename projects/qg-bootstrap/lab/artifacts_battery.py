...
def census():
    ...
    print(
        f"census: {sum(kinds.values())} excluded cells, {len(kinds)} distinct killers", flush=True
    )
...
def stack():
    files = [
        ("fig1_map_mu-9_5.json", F(-9, 5)),
        ("fig1_map_mu-6_5.json", F(-6, 5)),
        ("fig1_map_mu-3_5.json", F(-3, 5)),
        ("fig1_map_mu0_1_N40.json", F(0)),
        ("fig1_map_mu3_5.json", F(3, 5)),
        ("fig1_map_mu6_5.json", F(6, 5)),
    ...
    OUT["stack_comparison"] = {
        "total_points": total,
        "coarse_discrepancies": len(doomed),
        "all_discrepancies_are_map_allowed_analytic_doomed": all(
            d_["map"] and not d_["analytic"] for d_ in doomed
        ),
        "fine_mismatches": fine_mism,
        "doomed_cells": doomed,
    }
    print(
        f"stack: {total} points, {len(doomed)} doomed-cell discrepancies, "
        f"{fine_mism} fine mismatches",
        flush=True,
    )
...
def dichotomy():
    ...
    thr = {
        ...
        "k1_w17_10": {
            "sign_n84": 1 if a_route1(84, 83, F(-13, 25), F(17, 10), F(0)) > 0 else -1,
...
def qclock():
    ...
    for q in (
        ...
        table.append(
            {"q": str(q), "q_minus_1": float(q) - 1, "first_negative": first_negative_q(q, 60)}
        )
    ...
    OUT["q_clock"] = {
        "table": table,
        "q1_control_clean_to": 25 if ctrl is None else ctrl,
        "log_derivative_finite_diff_h": "1e-6",
        "g_values": gs,
    }
...
def d_checks():
    ...
    def a_gegen(n, l, r, w, mu0, D):
        ...
        return sp.nsimplify(
            sp.integrate(C * px * (1 - x**2) ** (al - sp.Rational(1, 2)), (x, -1, 1))
        )
    ...
    for D in (6, 10):
        ...
        checks.append({"D": D, "a_10_9_is_zero": bool(v0 == 0), "a_11_10_negative": bool(v1 < 0)})
...
def fixed_spin_200():
    ...
    for name, r, w in pts:
        for l in (0, 2):
            ...
            rows.append(
                {"point": name, "r": str(r), "w": str(w), "l": l, "signs_n50_100_200": signs}
            )
...
def main() -> int:
    ...
    d_checks()
    fixed_spin_200()
    (RES / "paper_artifacts.json").write_text(json.dumps(OUT, indent=1), encoding="utf-8")
