...
def check1():
    ...
    detail = (f"a_24,23={vals[24]} (>0? {vals[24] > 0}); "
...
def check2():
    ...
    while got < n_pts:
        ...
        ok_all &= agree
        details.append(f"(r={r},w={w}): sgn a20={sgn(a20)}, sgn law={sgn(law)}"
                       f" {'OK' if agree else 'MISMATCH'}")
    record(2, "a_{2,0} sign law at 10 random rational points", ok_all,
           "; ".join(details))
...
def check3():
    ...
    for i in range(9):
        ...
        ok_all &= ok
        details.append(f"w={w}: pos n<{nstar}:{pos_before}, "
                       f"a_{{{nstar},{nstar-1}}}==0:{marginal_zero}, "
                       f"first neg n={first_neg} (expect {nstar+1})"
                       f" {'OK' if ok else 'MISMATCH'}")
    record(3, "nine casualties at r=-3/5, first negative at n=10w+1", ok_all,
           "; ".join(details))
...
def check4():
    ...
    detail = (f"q=6/5 first neg {fn65} (expect (3,0)); "
...
def check5():
    ...
    ok = (a == 0)
    record(5, "D=6 exact zero a_{10,9} at (r,w)=(-3/5,1)", ok,
...
if __name__ == "__main__":
    ...
    print()
    print(f"TOTAL: {len(results)} items, {len(results) - n_fail} pass, "
          f"{n_fail} fail")
