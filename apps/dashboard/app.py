...
# ---------------------------------------------------------------- page 2
    ...
    for r in records:
        ...
        label = f"{r.id} · {r.source.identifier} · {r.support}" + (
        ...
        with st.expander(label):
            ...
            st.write(
                f"**Access:** full_text={r.access.full_text}, abstract_only={r.access.abstract_only}, "
            ...
            if r.limitations:
                ...
                for lim in r.limitations:
                    st.write(f"- {lim}")
            st.write(
                f"**Verified:** metadata={r.verified_by.metadata}, content={r.verified_by.content}, "
    ...
    for c in claims:
        ...
        if c.contradicting_evidence_ids:
            st.error(
                f"{c.id}: contradicting evidence {c.contradicting_evidence_ids} (kept visible)"
            )
...
# ---------------------------------------------------------------- page 3
    ...
    for h in hyps:
        ...
        st.caption(
            "What does this mean? A frozen hypothesis cannot be changed after seeing "
    ...
    if not exps:
        st.info(
            "No experiments yet. The manifest is created before the run and frozen before "
...
# ---------------------------------------------------------------- page 4
    ...
    for v in vals:
        icon = {"pass": "✅", "fail": "❌", "inconclusive": "❓", "pending": "⏳"}.get(
            v.decision.value, "⏳"
        )
        with st.expander(
            f"{v.id} · {icon} {v.decision.value} · independence: {v.independence_level}",
            expanded=True,
        ):
            st.write("**Checks:**")
            for name, state in v.checks.items():
                mark = {"pass": "✅", "fail": "❌", "pending": "⏳", "inconclusive": "❓"}.get(
                    state, state
                )
    ...
    st.caption(
        "What does this mean? The validator recomputes results with an independent "
...
# ---------------------------------------------------------------- page 5
    ...
    for c in claims:
        with st.expander(f"{c.id} · state: {c.state.value}", expanded=True):
            ...
            st.write(
                f"**Experiments:** {', '.join(c.experiment_ids) or 'none'} · "
            ...
            target = (
                order[idx + 1] if 0 <= idx < len(order) - 1 else order[0] if idx == -1 else None
            )
            if target:
                blockers = allowed_claim_promotion(c, target, records, vals)
                st.write(
                    f"**Promotion to `{target.value}` blocked by:**"
                    if blockers
        ...
        [
            {
                ...
                "one-week artifact": t.one_week_artifact,
                "compute": t.compute,
                "ceiling": t.scientific_ceiling,
                "publication": t.publication_path,
                "upstreams": ", ".join(t.required_upstreams),
            }
            for t in topics
        ],
...
# ---------------------------------------------------------------- page 7
    ...
    checks = [
        ...
        (
            "release checklist",
            any((proj / "release").glob("*.yaml")) if (proj / "release").is_dir() else False,
        ),
    ]
    ...
    st.caption(
        "Run `uv run sb release check " + project_id + "` for the full gate. "
