...
def build_steps(proj: Path) -> list[Step]:
    ...
    a1 = _j(st1)
    steps.append(Step("Instrument", "Research system built and clean-clone reproducible",
    ...
    steps.append(Step("Baseline", "Retrained a known black hole (Schwarzschild) as our ruler",
                      DONE if base_ckpt.exists() else (RUNNING if run_live else TODO),
                      f"500/500 epochs, final loss {loss:.2e}" if base_ckpt.exists() and loss
                      else (f"epoch {ep}/500" if run_live else "no checkpoint"),
                      "" if base_ckpt.exists() else "~3 h on CPU"))

    bstr = _j(proc / "baseline_stress.json")
    steps.append(Step("Baseline", "Measured how good that ruler actually is",
                      DONE if bstr else TODO,
                      f"hidden-point error: median {bstr['trained']['ricci_median']:.3f}, "
                      f"max {bstr['trained']['ricci_max']:.3f}" if bstr else "not measured"))

    frozen = proj / "validations" / "THRESHOLDS_FROZEN.md"
    steps.append(Step("Baseline", "Locked the pass/fail thresholds (before seeing candidates)",
                      DONE if frozen.exists() else BLOCKED,
                      "frozen + git tag `thresholds-frozen`" if frozen.exists()
    ...
    steps.append(Step("Candidates", "Trained candidate #1 from the authors' own config",
                      DONE if (raw / "petrovI-bh-run1" / "final_model.keras").exists()
    ...
    if cs1:
        ...
        steps.append(Step("Candidates", "Tested candidate #1 on points it never saw",
                          DONE, f"verdict {cs1['verdict_under_frozen_criteria']} — "
                                f"error median {h['median']:.3f} vs allowed 0.286"))
    bmap = _j(proc / "boundary_map.json")
    if bmap:
        steps.append(Step("Candidates", "Mapped where candidate #1 fails (horizon vs everywhere)",
                          DONE, f"{len(bmap['rows'])} grid points — failure is global, not boundary-local"))

    steps.append(Step("Candidates", "Trained candidate #2 (same config, different random seed)",
                      DONE if (raw / "petrovI-bh-run2" / "final_model.keras").exists() else TODO,
                      "500 epochs, seed 124" if cs2 else "not started", "" if cs2 else "~1.5 h"))
    if cs2:
        h2 = cs2["hidden_stats_h3e-3"]
        steps.append(Step("Candidates", "Tested candidate #2 the same way",
                          DONE, f"verdict {cs2['verdict_under_frozen_criteria']} — "
...
def render_eta_and_timeline(proj: Path) -> None:
    ...
    for s in steps:
        if s.stage != last_stage:
            rows.append(f'<div style="margin:14px 0 6px 0;font-size:0.8em;opacity:0.6;'
                        f'letter-spacing:0.1em;">{s.stage.upper()}</div>')
            last_stage = s.stage
        c = STEP_COLORS[s.status]
        icon = {DONE: "&#10004;", RUNNING: "&#9679;", WAITING: "&#9873;",
                BLOCKED: "&#9888;", NEXT: "&#9654;", TODO: "&#9675;"}[s.status]
        eta_badge = (f'<span style="font-size:0.8em;opacity:0.8;border:1px solid {c};'
